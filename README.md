...
This repo is a pnpm monorepo with two packages:
...
1. Add a changeset: `pnpm changeset` (pick `patch` for experiments; name
   whichever packages changed — the two are `linked`, so one changeset
   naming both ships them together).
